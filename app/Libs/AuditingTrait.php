<?php
//http://laravel-auditing.herokuapp.com/docs/2.4/examples
...
trait AuditingTrait
{
    ...
    {
        return $this->morphMany(SYSLogs::class, 'owner');
    }
    ...
    {
        return SYSLogs::where('owner_type', get_called_class())
        ...
        if ($this->isAuditEnabled()) {
            ...
            foreach ($this->updatedData as $key => $value) {
                if ($this->isAuditing($key)) {
                    if ($key == 'building_value') {
                        $log['new_value'][$key] = json_decode($value);
                    }
                    else {
        ...
        if ($this->isAuditEnabled() && $this->updating) {
            ...
            if (count($changes_to_record)) {
                foreach ($changes_to_record as $key => $change) {
                    if ($key == 'building_value') {
                        $log['old_value'][$key] = json_decode(array_get($this->originalData, $key));
                        $log['new_value'][$key] = json_decode(array_get($this->updatedData, $key));
                    }
                    else {
    ...
    public function auditDeletion()
    {
        ...
        if ($this->isAuditEnabled() && $this->isAuditing('deleted_at')) {
            ...
            foreach ($this->updatedData as $key => $value) {
                if ($this->isAuditing($key)) {
                    if ($key == 'building_value') {
                        $log['old_value'][$key] = json_decode($value);
                    }
                    else {
